...
export class RutasService {
    ...
    async getRutas(id: number, page: number) {
        ...
        return { status: 200, message: "Rutas fetched successfully", data: rutas, currentPage: page, totalPages };
    }

    async updateRuta(id: number, data: UpdateRutaDto){
        const existingRuta = await this.prismaService.rutaFront.findUnique({where: {id}});
        if(!existingRuta) throw new Error("Ruta not found");
        const updatedRuta = await this.prismaService.rutaFront.update({where: {id}, data});
        return { status: 200, message: "Ruta updated successfully", data: updatedRuta };
    }
